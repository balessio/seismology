...
with open('SETUP/SETUP_fetchFocalMechData.txt', 'r') as f:
...
for event in events:
    ...
    try:
        ...
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_rr)
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_tt)
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_pp)
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_rt)
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_rp)
        a=float(event.focal_mechanisms[0].moment_tensor.tensor.m_tp)
...
with open(TEXT_FILE, 'w') as textFile:
    ...
    for event in cleanEvents:
        textFile.write('\n'+str(event.preferred_origin().time)+'\n'+
                str(event.resource_id))

print('\n\nRuntime: '+str(UTCDateTime()-start)+' seconds\n\n')
os.system('say "Focal mechanisms collected"')
######################################################################
